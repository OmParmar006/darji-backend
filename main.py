...
async def run_vton_model(job_id: str, person_bytes: bytes, garment_bytes: bytes, garment_desc: str):
    try:
        ...
        def run_gradio():
            ...
            try:
                ...
                logger.info(f"[{job_id}] Raw result: {type(result)} — {str(result)[:200]}")
                return result
            finally:
        ...
        # Extract image from Gradio response (tuple of images)
        result_image = result[0] if isinstance(result, (list, tuple)) else result
        if isinstance(result_image, dict):
            result_image = result_image.get("url") or result_image.get("path") or str(result_image)
...
async def run_vton_two_pass(job_id: str, person_bytes: bytes, shirt_bytes: Optional[bytes], pant_bytes: Optional[bytes]):
    try:
...
@app.get("/api/tryon/test-hf")
async def test_hf():
    try:
        from gradio_client import Client
        hf_headers = {"Authorization": f"Bearer {HF_TOKEN}"} if HF_TOKEN else {}
        Client(HF_SPACE, headers=hf_headers)
        return {"status": "ok", "message": "HuggingFace Space connected"}
    except Exception as e:
...
async def submit_tryon(
    ...
    jobs[job_id] = {"job_id": job_id, "status": "pending", "result_url": None, "error": None, "created_at": datetime.utcnow().isoformat()}
...
async def get_status(job_id: str):
    ...
    return JSONResponse({"job_id": job["job_id"], "status": job["status"], "result_url": job.get("result_url"), "error": job.get("error")})


@app.delete("/api/tryon/cleanup")
async def cleanup_old_jobs():
    cutoff = datetime.utcnow() - timedelta(hours=1)
    removed = sum(1 for jid in list(jobs) if datetime.fromisoformat(jobs[jid]["created_at"]) < cutoff and jobs.pop(jid))
